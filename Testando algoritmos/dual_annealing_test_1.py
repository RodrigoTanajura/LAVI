...
samples = 50
speed_range = np.linspace(116, 122, samples) # rads/s
kxx = 1e6*0.92435
kyy = 1e6*1.14362
kxy = 1e6
cxx = 1e3*0.1234
...
plt.plot(speed_range, real_input([kxx,kyy,cxx,cyy]),'o')
plt.show()

print("K encontrado | esperado | erro(%) | erro original(%):")
for i in range(len(res.x)):
    print (trunc(res.x[i]), "|" , trunc(k[i]),"|",
            np.abs((k[i]-res.x[i])/(k[i]))*100, "|" ,
            np.abs((k[i]-x0[i])/(k[i]))*100)
