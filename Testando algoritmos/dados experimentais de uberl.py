...
arquivos = [r"C:\Users\diwiw\OneDrive\Documentos\A UFRJ\Estágio - LAVI\Dados de Uberlandia\TXT\0 Hz\Tentativa 3\D1S1S3\TRAC1.TXT",
...
def extract_data(arquivo):
    freq = np.loadtxt(arquivo)[:, 0]*2*np.pi
    amp = np.loadtxt(arquivo)[:, 1]
    im = np.loadtxt(arquivo)[:, 2]
    z = np.sqrt(amp**2 + im**2)
    return z[10:], freq[10:]

amp, freq = extract_data(arquivos[0])
pos = [ordem[0][0]*4+1,ordem[0][1]*4+1]
m1=0.14945 # massa do acoplamento na direcao x
m2=0.14945 # massa do acoplamento na direcao y
xxx = 1
...
def FRF_only_stiffness(k, speed_range, pos):
    ...
    K_mod[0*4+2,0*4+2] += xxx
    K_mod[0*4+3,0*4+3] += xxx
    K_mod[1*4+2,1*4+2] += xxx
    K_mod[1*4+3,1*4+3] += xxx
...
def objective_stiffness(k, speed_range, pos):
    res = (norm(z1 - FRF_only_stiffness(k, speed_range, pos), 2)**2)/(norm(z1, 2)**2)
    return res
